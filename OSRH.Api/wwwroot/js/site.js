...
async function verifyDocument(docId, status) {
    ...
    try {
        // FIXED: Changing 'admin' to 'operator' to match your new AppController
...
async function addDriverAvailability() {
    const currentUser = sessionStorage.getItem('currentUser');
    ...
    try {
        const response = await fetch('/api/app/driver/add-availability', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username: currentUser, weekday: day, start: start, end: end, notes: 'Web App' })
        });
        
        if (response.ok) {
            alert("Η βάρδια προστέθηκε!");
            loadAvailability(); // Reload list
        } else {
            alert("Σφάλμα.");
        }
...
async function checkActiveTrip() {
    ...
    if (data.length > 0) {
        const trip = data[0];
        container.innerHTML = `
            <p><strong>Επιβάτης:</strong> ${trip.PassengerName}</p>
        ...
        container.innerHTML = "Δεν υπάρχει ενεργή διαδρομή.";
        btnStart.classList.add('hidden');
        btnEnd.classList.add('hidden');
    }
}
...
async function updateTripStatus(tripId, status) {
    if(!confirm(`Αλλαγή κατάστασης σε: ${status}?`)) return;
...
function resetView() {
    ...
    // 3. NEW: Hide the "Scanning..." and "Shift" screens
...
async function fetchData(endpoint) {
    ...
    try {
        ...
        if (filteredData.length > 0) {
            ...
            const isAdminDocView = endpoint.includes('operator/pending-documents'); 
            if (isDriverView || isAdminDocView) {
                const th = document.createElement("th");
            ...
            filteredData.forEach(row => {
                ...
                if (isAdminDocView) {
                    ...
                    td.style.display = "flex"; td.style.gap = "5px";
                    const btn1 = document.createElement("button");
                    btn1.innerText = "OK"; btn1.style.backgroundColor = "#28a745";
                    btn1.onclick = () => verifyDocument(row.document_id, 'Verified');
                    const btn2 = document.createElement("button");
                    btn2.innerText = "Reject"; btn2.style.backgroundColor = "#dc3545";
...
async function submitShift() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            loadAvailability(); // Go back to the list view
        } else {
...
async function updateTripStatus(tripId, status) {
    ...
    showActiveTripScreen(); // Refresh the screen
}
